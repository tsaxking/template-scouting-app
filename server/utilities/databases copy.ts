...
{
    const cannotConnect = 'Cannot connect to the database:';

    if (!DATABASE_USER) throw new Error(`${cannotConnect} .env:DATABASE_USER is not defined`);
    if (!DATABASE_PASSWORD) {
        throw new Error(`${cannotConnect} .env:DATABASE_PASSWORD is not defined`);
    }
    if (!DATABASE_NAME) throw new Error(`${cannotConnect} .env:DATABASE_NAME is not defined`);
    if (!DATABASE_HOST) throw new Error(`${cannotConnect} .env:DATABASE_HOST is not defined`);
    if (!DATABASE_PORT) throw new Error(`${cannotConnect} .env:DATABASE_PORT is not defined`);
}
