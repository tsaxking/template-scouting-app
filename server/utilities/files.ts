...
const matchJSONError = (e: Error): JSONError => 
    matchInstance<Error, JSONError>(e,
         [SyntaxError, () => 'InvalidJSON'],
         [Error, () => 'Unknown']) ?? 'Unknown';

const matchFileError = (e: Error): FileError =>
    matchInstance<Error, FileError>(e,
        [Error, () => 'Unknown'],
        [TypeError, () => 'NoFile'],
        [Error, () => 'FileExists'],
        [Error, () => 'NoAccess']) ?? 'Unknown';
...
export const getJSONSync = <T = unknown>(file: string): Result<T, JSONError> => {
    return attempt(() => {
...
export const getJSON = <T = unknown>(file: string): Promise<Result<T, JSONError>> => {
...
export const saveJSONSync = <T = unknown>(file: string, data: T): Result<void, JSONError> => {
    return attempt(() => {
        const filePath = filePathBuilder(file, '.json', './storage/jsons');
...
export const saveJSON = <T = unknown>(file: string, data: T): Promise<Result<void, JSONError>> => {
    return attemptAsync(async () => {
        const filePath = filePathBuilder(file, '.json', './storage/jsons');
        makeFolder(path.dirname(filePath));
...
export const getTemplateSync = (file: string, options?: Constructor): Result<string, FileError> => {
    return attempt(() => {
...
export const getTemplate = (file: string, options?: Constructor): Promise<Result<string, FileError>> => {
    return attemptAsync(async () => {
        const p = filePathBuilder(file, '.html', './public/templates');
...
export const saveTemplateSync = (file: string, data: string): Result<void, FileError> => {
    return attempt(() => {
        const p = filePathBuilder(file, '.html', './public/templates');
...
export const saveTemplate = (file: string, data: string): Promise<Result<void, FileError>> => {
    return attemptAsync(async () => {
        const p = filePathBuilder(file, '.html', './public/templates');
        makeFolder(path.dirname(p));
...
export const removeTemplate = (file: string): Promise<Result<void, FileError>> => {
    return attemptAsync(async () => {
        const p = filePathBuilder(file, '.html', './public/templates');
...
export const saveUpload = (filename: string, data: Uint8Array): Promise<Result<void, FileError>> => {
    return attemptAsync(async () => {
        const p = path.resolve(__uploads, filename);
        makeFolder(path.dirname(p));
        await
            fs.promises.writeFile(p
                , data);
    }
    , matchFileError);
}

export const saveUploadSync = (filename: string, data: Uint8Array): Result<void, FileError> => {
    return attempt(() => {
        const p = path.resolve(__uploads, filename);
        makeFolder(path.dirname(p));
        fs.writeFileSync(p, data);
...
export const removeUpload = (filename: string): Promise<Result<void, FileError>> => {
    return attemptAsync(async () => {
        const p = path.resolve(__uploads, filename);
        await fs.promises.rm(p);
    }, matchFileError);
...
export const getUpload = (filename: string): Promise<Result<Uint8Array, FileError>> => {
    return attemptAsync(async () => {
        const p = path.resolve(__uploads, filename);
...
export const getUploadSync = (filename: string): Result<Uint8Array, FileError> => {
    return attempt(() => {
        const p = path.resolve(__uploads, filename);
...
export const saveFile = (file: string, data: string | ArrayBuffer): Promise<Result<void, FileError>> => {
    return attemptAsync(async () => {
        const p = path.resolve(__root, file);
        makeFolder(path.dirname(p));
...
export const saveFileSync = (file: string, data: string): Result<void, FileError> => {
    return attempt(() => {
        const p = path.resolve(__root, file);
        makeFolder(path.dirname(p));
        fs.writeFileSync(p, data);
