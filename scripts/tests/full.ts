...
const err = (...data: unknown[]) => console.error(
    Colors.FgRed,
    '[Test]',
    Colors.Reset,
    ...data
...
const readEnv = (envPath: string): Env => {
    ...
    return env.split('\n').reduce((acc, line) => {
        const [key, value] = line.split('=').map(k => k.trim().replace(/['"]/g, ''));
        if (key && value) {
            acc[key] = value;
        }
        return acc;
...
const saveEnv = (envPath: string, env: Env) => {
    const envStr = Object.keys(env).map(key => `${key} = '${env[key]}'`).join('\n');
    ...
    start() {
        return new Promise<void>((res, rej) => {
            ...
            this.process.stdout?.on('data', (data) => {
                ...
                if (data.toString().includes('Server is listening on port 3000')) {
                    this.status = 'on';
                    res();
                }
            });
...
export const runTest = (url: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE', body: unknown) => {
