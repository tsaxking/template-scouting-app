...
export const merge = async (num: number): Promise<Result<unknown>> => {
    return attemptAsync(async () => {
        ...
        console.log(mergeTsQueries, mergeTsTables, currentTsQueries, currentTsTables);

        const doMerge = async (name: string, from: Record<string, unknown>, to: Record<string, unknown>): Promise<unknown> => {
            const result = { ...to };
            for (const [key, value] of Object.entries(from)) {
                if (result[key] && JSON.stringify(result[key]) !== JSON.stringify(value)) {
        ...
        const imports: string[] = mergedQueries.map((q) => {
        ...
        Deno.writeTextFileSync('server/utilities/merge-tables.ts', warning + tableTs);
        Deno.writeTextFileSync('server/utilities/merge-queries.ts', warning + importTs + '\n\n' + queryTs);
    });
}
