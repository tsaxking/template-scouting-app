...
export const parseTsTypes = async (file: string): Promise<
    Result<{
        [key: string]: QueryObjectType;
    }>
> => {
...
export const parseTsQueries = async (file: string): Promise<
    Result<{
        name: string;
        params: string[];
        returns: string;
...
> => {
    return attemptAsync(async () => {
        ...
        const allQueries = exports.replace(
            /export(\s+)type(\s+)Queries(\s+)=(\s+){/g,
            '',
        ).replace(/}/g, '').split(splitter);
        ...
        return allQueries.map((q) => {
            ...
            const params = firstMatch(q.match(paramRegex)).split(',').map(
                (s) => {
                    if (s.includes('{')) s += '}';
                    return s.replace(/\[|\n|\s/g, '').trim();
                },
...
export const writeFiles = async (queryDir: string, tableDir: string, tables: {
    [key: string]: QueryObjectType;
    ...
    for (const t of Object.keys(tables)) {
        importTs += `import { ${t} } from '${
            relative(resolve(tableDir, queriesName), resolve(queryDir, tablesName)).slice(1)
        }';\n`;
    }
