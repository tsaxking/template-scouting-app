...
import { repeatPrompt, confirm } from '../prompt.ts';
...
const restore = async () => {
    ...
    if (db.isOk()) {
        ...
        makeBackup(
            new Database(
                resolve(
                    __root, './storage/db', env.DATABASE_LINK + '.db'
                )
            )
        );
...
const newVersion = async () => {
    ...
    if (!DB.hasVersion(latest)) {
        backToMain('Database must be at the latest version, please restore the latest backup or update the database');
        return;
    }

    
    const m = +repeatPrompt('Enter the minor version number', undefined, (data) => !isNaN(parseInt(data)), false);
    const p = +repeatPrompt('Enter the patch version number', undefined, (data) => !isNaN(parseInt(data)), false);

    const version: [number, number, number] = [1, m, p];


    if (DB.hasVersion(version)) {
        backToMain(`Version ${version.join('.')} already implemented (Current Version: ${DB.version.join('.')})`);
        return;
    }
