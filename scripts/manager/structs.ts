...
import fs from 'fs';
import { Blank, Data, DataVersion, Struct, Structable } from "../../server/structure/structs/struct";
...
import { SQL_Type } from "../../client/models/struct";
...
const openStructs = () => {
    return attemptAsync<Struct<Blank, string>[]>(async () => {
        const readDir = async (dir: string): Promise<Struct<Blank, string>[]> => {
            ...
            const res = await Promise.all(contents.map(async object => {
        ...
        return await readDir(path.resolve(__root, 
            'server/structure/structs',
        ));
    });
};

const selectStruct = async (message?: string, structs?: Struct<Blank, string>[]) => {
...
const selectData = async (data: Data<Struct<Blank, string>>[], message?: string) => {
    const run = async () => {
...
const versionActions = {
    restore: async (version: DataVersion<Blank, string>) => {
        const res = await confirm('Are you sure you want to restore this version?');
        if (res) {
    ...
    delete: async (version: DataVersion<Blank, string>) => {
        const res = await confirm('Are you sure you want to delete this version?');
        if (res) {
...
const dataActions: {
    ...
    update: async (data) => {
        const properties = Object.entries(data.data)
            // ignore global cols
            .filter(([e]) => !['id', 'archived', 'created', 'updated', 'universes', 'attributes'].includes(e));

        const toUpdate: Record<string, unknown> = {};

        for (const p of properties) {
            const [name, current] = p;
            const type = data.struct.data.structure[name];
    ...
    versionHistory: async (data) => {
        ...
        if (history === 'not-enabled') {
            return backToStruct(`Version history not enabled on ${data.struct.name}`);
        }
    ...
    addAttributes: async (data) => {
        ...
        const separated = attributes.split(',').map(d => removeWhitespace(d.trim()));
    ...
    removeAttribute: async (data) => {
        ...
        const res = await confirm(`Are you sure you want to remove ${attribute} from this?`);
    ...
    setAttributes: async (data) => {
        ...
        const res = await confirm(`Are you sure you want to overwrite the current attributes (${data.getAttributes().unwrap()} and overwrite them with ${separated.join(', ')}`);
    ...
    setUniverses: async (data) => {
        ...
        const run = async () => {
            ...
            if (res === '$$done$$') {
                const sure  = await confirm('Are you sure you want to set these universes?');
                if (sure) {
    ...
    removeFromUniverse: async (data) => {
        const currentIds = data.getUniverses().unwrap();
        const has = (await Permissions.Universe.all()).unwrap()
        ...
        if (selected) {
            const res = await confirm(`Are you sure you want to remove this from ${selected.data.name}?`);
...
const checkStrType = (str: string, type: SQL_Type): boolean => {
    switch (type) {
        case 'text':
            return true;
        case 'integer':
...
const returnType = (str: string, type: SQL_Type) => {
    return match(type)
        .case('text', () => str)
        .case('integer', () => +str)
        .case('numeric', () => +str)
...
const structActions: {
    ...
    new: async (struct) => {
        ...
        console.log(removeWhitespace(`
            ...
            If something is a date, use the format YYYY-MM-DD HH:MM:SS:ms
        `.trim()));
    ...
    deleteImage: async (struct) => {
        const sure = await confirm(`Are you sure you want to delete the image for ${struct.name}?`);
        if (sure) {
    ...
    drop: async (struct) => {
        const sure = await confirm(`Are you sure you want to delete ${struct.name}?`);
        if (sure) {
    ...
    clear: async (struct) => {
        const sure = await confirm(`Are you sure you want to clear ${struct.name}?`);
        if (sure) {
            (await struct.clear()).unwrap();
