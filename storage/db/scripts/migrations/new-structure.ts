...
export default new Version(
    ...
    async (db) => {
        ...
        await (async () => {
            ...
            const notifications = (await db.all('account-notifications/all')).unwrap();
            (await db.unsafe.run(db.Query.build(`DROP TABLE Accounts;`))).unwrap();
            (await db.unsafe.run(db.Query.build(`DROP TABLE AccountNotifications;`))).unwrap();

            // There isn't much implementation for settings right now, so we're just going to drop it.
            (await db.unsafe.run(db.Query.build('DROP TABLE AccountSettings;'))).unwrap();
    
            const built = resolveAll(await Promise.all([
            ...
            await Promise.all(account.map(async a => {
                const newAccount = (await Account.Account.new({
                    username: a.username,
                    key: a.key,
                    salt: a.salt,
                ...
                if (a.passwordChange && a.passwordChangeDate) {
                    Account.PasswordChange.new({
                        ...
                        expires: new Date(a.passwordChangeDate).toISOString(),
                    });
                }
                ...
                const notifs = notifications.filter(n => n.accountId === a.id);
        ...
        // Sessions
        await (async () => {
            const sessions = (await db.all('sessions/all')).unwrap()
                .filter(s => !!s.accountId); // ignore sessions that aren't signed in
            
            (await db.unsafe.run(db.Query.build(`DROP TABLE Sessions;`))).unwrap();
            ...
            await Promise.all(sessions.map(async s => {
                const newSession = (await Session.Session.new({
