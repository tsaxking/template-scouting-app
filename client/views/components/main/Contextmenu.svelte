...
{#if showMenu}
    <div class="card rounded shadow border-0" use:getContextMenuDimension style="position: absolute; top:{pos.y}px; left:{pos.x}px">
                ...
                {#each menuItems as item}
                    ...
                    {:else}
                        <li class="list-group-item border-0 p-0 w-100">
                            <button on:click={item.onClick} class="border-0 btn btn-dark w-100 text-start"><i class={item.class}></i>{item.displayText}</button>
                        </li>
                    {/if}
                {/each}
            </ul>
