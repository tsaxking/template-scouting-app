    ...
    const fns = {
        onInput: async (e: Event) => {
            ...
            if (files) {
                pictures = [
                ...(multiple ? pictures : []),
                ...await Promise.all(Array.from(files).map(f => new Promise<P>((res, rej) => {
        ...
        {#each pictures as picture, i}
            <div class="col-sm-4 col-md-3 col-lg-2 m-1 no-select">
                <div class="d-flex justify-content-between align-items-center py-1">
                    ...
                    <i class="material-icons text-danger cursor-pointer p-0 m-0" on:click={() => fns.remove(picture)}>
                        close
                    </i>
                </div>
                <img src={picture.url} alt="picture {i}" class="img-thumbnail p-1">
            </div>
        {/each}
    </div>
</div>