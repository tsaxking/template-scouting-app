...
<div class="row m-3"> 
    {#if type === 'range'}
        <div class="col-{labelwidth} d-flex justify-content-center align-items-center">
            <label for={ids.range} class="form-label m-0">{name}
                <!-- <span class="badge bg-secondary ms-2">{value !== undefined ? value : ''}</span> -->
            </label>
        </div>
        <div class="col-{inputWidth}">
            <input type="range" min={options[0] || 0} max={options[1] || 100} class="form-range" id={ids.range} bind:value={value} on:change={(e) => {
    ...
    {:else if type === 'switch'}
        ...
        <div class="col-{inputWidth}">
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id={ids.switch} bind:checked={value} on:change={(e) => {
    ...
    {:else if type === 'select'}
        ...
        <div class="col-{inputWidth}">
            <select id={ids.select} bind:value={value} on:change={(e) => {
                onChange(e.currentTarget.value);
            }} class="form-select">
