...
const fns = {
    ...
    matchInfo: async () => {
        ...
        save.addEventListener('click', () => {
            ...
            const match = eventData.matches.find(
                m =>
                    m.comp_level === data.compLevel &&
                    m.match_number === data.matchNum
            );
            if (!match) {
                console.error(
                    'Could not find match, assigning alliance to null'
                );
                return (App.matchData.alliance = null);
            }
            const alliance = match.alliances.red.team_keys.includes(
                `frc${data.teamNum}`
            )
                ? 'red'
                : 'blue';
    ...
    assignedTeams: () => {
        ...
        body.$on('group', async e => {
            ...
            App.matchData.alliance = match.alliances.red.team_keys.includes(
                `frc${App.matchData.teamNumber}`
            )
                ? 'red'
                : 'blue';
        });
