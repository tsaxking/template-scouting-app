...
const scaleMatrix = [.5, .5, .5] as Point3D;
const translateMatrix = [.25, .25, .25] as Point3D;
...
c.animate(() => {
    ...
    for (const [p1, p2] of edges) {
        ...
        const fromColor = new Color(...circles[p1].center as Point3D).toString('rgba');
        const toColor = new Color(...circles[p2].center as Point3D).toString('rgba');
        ...
        c.$ctx.beginPath();
        c.$ctx.moveTo(circles[p1].x * c.$ctx.canvas.width, circles[p1].y * c.$ctx.canvas.height);
        c.$ctx.lineTo(circles[p2].x * c.$ctx.canvas.width, circles[p2].y * c.$ctx.canvas.height);
        c.$ctx.stroke();
        c.$ctx.restore();
    }
    ...
    for (const p of splinePoints) {
        ...
        p.y = a[1];
        p.z = a[2];
        const color = new Color(...p.array.map(n => n*255) as Point3D).toString('rgba');

        c.$ctx.fillStyle = color;
        c.$ctx.beginPath();
        c.$ctx.arc(p.x * c.$ctx.canvas.width, p.y * c.$ctx.canvas.height, 5, 0, 2 * Math.PI);
        c.$ctx.fill();
    }
});
