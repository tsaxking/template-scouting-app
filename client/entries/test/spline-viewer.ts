...
resetBtn.classList.add('btn', 'btn-secondary', 'position-absolute', 'top-0', 'start-0', 'm-2');
resetBtn.innerText = 'Reset';
document.body.appendChild(resetBtn);

const downloadBtn = document.createElement('button');
downloadBtn.classList.add('btn', 'btn-secondary', 'position-absolute', 'top-0', 'end-0', 'm-2');
...
const start = () => {
    ...
    c.add(...Object.values(paths).map(p => {
        p.$properties.line = {
...
downloadBtn.onclick = () => {
    downloadText(JSON.stringify(new Array(1000).fill(0).map((_, i) => spline.ft(i / 1000))), 'spline.json');
}
