 ...
 */
export const destructureMatch = (match: TBAMatch): [number, number, number, number, number, number] => {
    return [
 ...
 */
export const generateScoutGroups = (teams: TBATeam[], matches: TBAMatch[]): Assignment => {
    let interferences = 0;

    // only use qualification matches
    matches = matches.filter(m => m.comp_level === 'qm').sort((a, b) => a.match_number - b.match_number);
    teams = teams.sort((a, b) => a.team_number - b.team_number);

    // unique teams for each scout group
    const scoutGroups: TBATeam[][] = new Array(6).fill(0).map(() => new Array<TBATeam>());
    ...
    const conflicts = new Array(matches.length).fill(0).map(() => new Array<TBATeam>());

    const scoutLists = scoutGroups.map((scoutTeams, ti, ta) => {
        return matches.map((m, mi) => {
            const mTeams = destructureMatch(m);
            const teams = scoutTeams.filter(t => mTeams.includes(t.team_number));
...
export const testAssignments = (assignment: Assignment): Status => {
    ...
    for (let i = 0; i < groups.length; i++) {
        ...
        if (unique.size !== teams.length) return {
        ...
        for (let j = 0; j < groups.length; j++) {
            if (i === j) continue;
            if (teams.some(t => groups[j].includes(t))) return {
    ...
    const isCorrectLength = matchAssignments.every(m => m.length === matchAssignments[0].length);
    ...
    for (let i = 0; i < matchAssignments.length; i++) {
        ...
        for (let j = 0; j < matchAssignments.length; j++) {
            if (i === j) continue;
            if (matches.some((t, i) => matchAssignments[j][i] === t)) return {
